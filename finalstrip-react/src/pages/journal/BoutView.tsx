...
export function BoutView() {
    ...
    const getData = async () => {
        if (dataReceived === false) {
            const result = await axios.get('api/bouts/detail/' + slug, {withCredentials: true})
    ...
    function boutSummaryDetail() {     
        return(
            <div className='container'>
                <p className="detail">{bout.fencerA ? bout.fencerA : "Unknown"} vs {bout.fencerB ? bout.fencerB : "Unknown"}</p>
                <p className="detail">{bout.scoreA} - {bout.scoreB}</p>
